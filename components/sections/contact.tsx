    ...
    <section id="kontakt" className="section-space bg-[var(--deep-navy)] text-white">
      <div className="site-container grid gap-10 lg:grid-cols-[1fr_auto] lg:items-end">
        ...
        <div className="flex flex-col gap-3 sm:flex-row lg:flex-col">
          <Link className="button-primary" href="#kontakt-start">Problem beschreiben</Link>
          <Link className="button-secondary button-secondary-dark" href="#kontakt-start">Erstgespräch vereinbaren</Link>
        </div>
      </div>
      <div id="kontakt-start" className="site-container mt-12 grid gap-4 border-t border-white/20 pt-8 text-sm text-slate-300 sm:grid-cols-3">
        <p><span className="block text-xs uppercase tracking-[0.14em] text-blue-200">E-Mail</span>{siteConfig.email}</p>
        <p><span className="block text-xs uppercase tracking-[0.14em] text-blue-200">Telefon</span>{siteConfig.phone}</p>
